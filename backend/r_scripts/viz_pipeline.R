...
# Runs when bray_curtis_distance_matrix.csv is present — safe no-op otherwise
tryCatch({
  ...
  extra_script <- file.path(script_dir, "dada2_extra_viz.R")
  if (file.exists(extra_script)) {
    source(extra_script, local=TRUE)
    dada2_extra_viz(OUTPUT_DIR)
  }
}, error=function(e) cat("  [skip] DADA2 extra viz:", e$message, "\n"))

# =============================================================================
#  DADA2 EXTRA VIZ — source shared helper and run when DADA2 outputs detected
# =============================================================================
tryCatch({
...
# (inline fallback in case source fails — defines the same function)
dada2_extra_viz <- function(out_dir) {
  ...
  have_vegan <- suppressPackageStartupMessages(
    tryCatch({ library(vegan); TRUE }, error=function(e) {
      cat("  [skip all] vegan not available:", e$message, "\n"); FALSE }))

  # ── 1. PCoA from Bray-Curtis ────────────────────────────────────────────────
  tryCatch({
    if (file.exists(file.path(out_dir, "pca_scores.csv"))) {
      cat("  (pca_scores.csv already exists — skip)\n")
    } else {
      ...
      pc_df  <- data.frame(
        ...
        stringsAsFactors = FALSE)
      write.csv(pc_df, file.path(out_dir, "pca_scores.csv"), row.names=FALSE)
      cat("  ✓ pca_scores.csv\n")
  ...
  if (have_vegan) tryCatch({
    if (file.exists(file.path(out_dir, "nmds_bray.csv"))) {
      cat("  (nmds_bray.csv already exists — skip)\n")
    } else {
      dm     <- read.csv(bray_file, row.names=1, check.names=FALSE)
      dm_mat <- as.matrix(dm)
      set.seed(42)
      ...
      nm_df  <- data.frame(Sample=rownames(dm_mat), NMDS1=nm$points[,1],
                            ...
                            stringsAsFactors=FALSE)
      write.csv(nm_df, file.path(out_dir, "nmds_bray.csv"), row.names=FALSE)
      cat("  ✓ nmds_bray.csv  (stress =", round(nm$stress, 4), ")\n")
    }
  }, error=function(e) cat("  [skip] NMDS Bray:", e$message, "\n"))

  # ── 3. Jaccard matrix → Heatmap + NMDS ──────────────────────────────────────
  asv_t_cached <- NULL
  if (have_vegan && file.exists(asv_file)) tryCatch({
    ...
    if (!file.exists(file.path(out_dir, "jaccard_heatmap.csv"))) {
      jac_mat <- as.matrix(vegdist(asv_t_cached, method="jaccard", binary=FALSE))
      jac_df  <- as.data.frame(jac_mat)
      write.csv(jac_df, file.path(out_dir, "jaccard_heatmap.csv"))
      ...
      cat("  (jaccard_heatmap.csv already exists — skip)\n")
    }
  }, error=function(e) cat("  [skip] Jaccard:", e$message, "\n"))

  # ── 4. Rarefaction curves (wide format: Depth, Sample1, Sample2, ...) ───────
  if (have_vegan && file.exists(asv_file)) tryCatch({
    if (file.exists(file.path(out_dir, "rarefaction.csv"))) {
      cat("  (rarefaction.csv already exists — skip)\n")
    } else {
      ...
      for (i in seq_len(nrow(asv_t_cached))) {
        ...
        rich  <- suppressWarnings(
          rarefy(asv_t_cached[i, , drop=FALSE], sample=depths))
        wide_df[[sname]] <- round(as.numeric(rich), 1)
      }
      write.csv(wide_df, file.path(out_dir, "rarefaction.csv"), row.names=FALSE)
  ...
  # ── 5. ASV length distribution (from taxonomy_table.csv) ────────────────────
  if (file.exists(tax_file)) tryCatch({
    if (file.exists(file.path(out_dir, "asv_lengths.csv"))) {
      cat("  (asv_lengths.csv already exists — skip)\n")
    } else {
      ...
      write.csv(len_tbl, file.path(out_dir, "asv_lengths.csv"), row.names=FALSE)
      cat("  ✓ asv_lengths.csv  (range:", min(lengths), "-", max(lengths), "bp,",
          length(asv_seqs), "ASVs)\n")
    }
